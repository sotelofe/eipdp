...
@Component({
  selector: 'app-emitirdictamen',
  templateUrl: './emitirdictamen.component.html',
  styleUrls: ['./emitirdictamen.component.scss']
})
export class EmitirdictamenComponent implements OnInit {
  ...
  enviar(){
    ...
    this.spinner.show();
    this.flujoService.subirDictamen(this.documento).subscribe(
      result => {

            this.response = result;
            if (this.response !=null ) {
                if(this.response.estatus === "ok"){
                  this.toastr.success('El dictamen se ha enviado de forma correcta', 'Aviso', { positionClass: 'toast-top-full-width', closeButton: true });
                }else if(this.response.estatus === "fail"){
                  this.frameFail.show();
                }else{
                  this.toastr.warning('No se pudo enviar tu dictamen', 'Error', { positionClass: 'toast-top-full-width', closeButton: true });
                }
            } else {
