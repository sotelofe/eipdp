...
@Component({
  selector: 'app-solicitarinforme',
  templateUrl: './solicitarinforme.component.html',
  styleUrls: ['./solicitarinforme.component.scss']
})
export class SolicitarinformeComponent implements OnInit {
  ...
  enviar(){
    ...
    this.spinner.show();
    this.flujoService.solicitarInforme(this.documento).subscribe(
      result => {

            this.response = result;
            if (this.response !=null ) {
                if(this.response.estatus === "ok"){
                  this.toastr.success('La solicitud de informe se ha enviado de forma correcta', 'Aviso', { positionClass: 'toast-top-full-width', closeButton: true });
