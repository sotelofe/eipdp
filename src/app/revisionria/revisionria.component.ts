...
@Component({
  ...
  templateUrl: './revisionria.component.html',
  styleUrls: ['./revisionria.component.scss']
})
export class RevisionriaComponent implements OnInit {
  ...
  precargaRia2(){
    ...
    this.spinner.show();
          ...
          this.flujoService.obtenerDetalleRiaFlujo12(obj).subscribe(
            result => {
  ...
  descargareRia(pregunta){
    this.serviceDescarga.descargaDocumento(this.upload2.filename ,1, this.rutaService.ruta(this.folioConsultafUno),pregunta);
  }

  aceptar(){ 
    this.acceptarFuno.folio = this.folioConsultafUno;
   
    this.flujoService.aceptarRiaUno(this.acceptarFuno).subscribe(
      result => {
